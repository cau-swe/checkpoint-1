...
| 사용자에게 누락된 정보가 있다고 알려준다.             |  D  | Notifier             |  
...
| Controller ↔ PageMaker          | Controller는 InputPage를 만들도록 요청하거나 UpdatedPage를 만들도록 요청한다. 그리고 만들어진 page를 받는다. | conveys request |
| PageMaker  ↔ InputPage          | PageMaker는 InputPage를 만든다.                               | prepares        |
| PageMaker  ↔ UpdatedPage        | PageMaker는 UpdatedPage를 만든다.                             | prepares        |
| StoreRequest ↔ Controller       | Controller는 입력된 피자정보 저장에 대한 요청을 전달받는다.    | receives        |
...
| Contrlloer ↔ BlankChecker       | Controller는 BlankChecker에게 input parameter에 빈칸이 있는지를 검사하도록 시킨다. | detects blank |
| BlankChecker ↔ Notifier         | BlankChecker는 
